<?php
	...
	if (array_key_exists('_submit_check',$_POST) && !empty($_POST['titulo'])) { 
		...
		Post::update($this->id,$title,$text);
		echo "<a href='../post/". $this->id ."'>Post</a> updated!";
	...
	<script src="../vendors/EpicEditor/js/epiceditor.js"></script>
		...
		<form class="pure-form" class="pure-form" method="post" action="../editPost/<?php echo $this->id; ?>">
			<p class="form">Post Title:</p>
			<p><input style="width:100%;" type="text" name="titulo" value="<?php echo $this->post["titulo"]; ?>" /></p>
			<p class="form">Text:</p>
			<div id="epiceditor" style="height: 350px;"></div>
			<p><textarea style="display:none;" name="texto" id="texto" rows="20" cols="40"><?php echo $this->post["text"]; ?></textarea></p>
			<p><input type="hidden" name="_submit_check" value="1"/> </p>
			<p><input class="pure-button" type="submit" name="submitted" value="Update Post" /></p>
		</form>
